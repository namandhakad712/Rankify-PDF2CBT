...
const valIssues = computed<Issue[]>(() => {
  ...
  paper.value.questions.forEach((qq, i) => {
    ...
    if (!ans) out.push({ index: i, level: "error", msg: `Q${qq.number} — answer is empty` })
    if (qq.type === "mcq" && (!qq.options || qq.options.length === 0)) out.push({ index: i, level: "error", msg: `Q${qq.number} — MCQ has no options` })
    if (qq.type === "msq" && (!qq.answers || qq.answers.length === 0)) out.push({ index: i, level: "error", msg: `Q${qq.number} — MSQ answers empty` })
    if (qq.type === "nat" && ans && isNaN(Number(ans))) out.push({ index: i, level: "warn", msg: `Q${qq.number} — NAT answer is not numeric` })
    if (!qq.subject) out.push({ index: i, level: "warn", msg: `Q${qq.number} — no subject set` })
  })
  return out
})
      ...
      <div class="max-w-[1700px] mx-auto px-5 pt-24 pb-4">
        ...
        <div class="sticky top-[88px] z-30 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white px-4 py-2.5 shadow-[0_14px_40px_-22px_rgba(35,32,58,0.35)] ring-1 ring-ink/[0.06]">
          <div class="flex items-center gap-3 min-w-0">
            <button class="text-sm font-medium text-ink/50 hover:text-ink transition-colors shrink-0" @click="router.push('/extract')">← back</button>
            <span class="h-5 w-px bg-ink/10 shrink-0"></span>
            <h1 class="font-display font-extrabold tracking-tight text-xl shrink-0" :title="pdfName ?? undefined">Review</h1>
            <span v-if="pdfName" class="font-hand text-lg text-ink/40 truncate max-w-52 hidden xl:inline">{{ pdfName }}</span>
          </div>
          <div class="flex items-center gap-2.5">
            <span v-if="dirtyCount" class="font-mono text-[11px] font-bold px-2.5 py-1 rounded-full bg-hlyellow/60 text-ink">{{ dirtyCount }} edited</span>
            <button :class="['px-3.5 py-2 rounded-xl border-2 text-xs font-bold transition-colors', showRendered ? 'border-pen/40 bg-pen/[0.06] text-pen' : 'border-ink/12 text-ink/60 hover:text-ink']" @click="showRendered = !showRendered">{{ showRendered ? "◉ Rendered" : "◌ Raw" }}</button>
            <button class="px-5 py-2 rounded-xl bg-pen text-white text-sm font-bold transition-transform hover:-translate-y-0.5" @click="saveAndGoTest">Save &amp; start test →</button>
            <button :class="['px-3.5 py-2 rounded-xl border-2 text-xs font-bold transition-colors', showPanel ? 'border-pen/40 bg-pen/[0.06] text-pen' : 'border-ink/12 text-ink/60 hover:text-ink']" @click="showPanel = !showPanel">Validation</button>
          </div>
        </div>
        ...
        <div class="mt-3 flex gap-4 h-[calc(100vh-11.5rem)] min-h-[420px]">
          ...
          <aside class="rev-zone hidden lg:flex w-72 shrink-0 flex-col rounded-xl bg-white shadow-[0_14px_40px_-22px_rgba(35,32,58,0.3)] ring-1 ring-ink/[0.06] overflow-hidden">
            <div class="p-3 border-b border-ink/[0.08] space-y-2.5">
              ...
              <label class="flex items-center gap-2 text-xs font-medium text-ink/65 cursor-pointer select-none">
                <input type="checkbox" v-model="diagramsOnly" class="accent-pen" /> Diagrams only
              </label>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-2 space-y-1.5">
              ...
              <p v-if="!filteredQuestions.length" class="text-xs text-ink/45 text-center py-6">No questions match this filter.</p>
            </div>

            <div class="px-3 py-2 border-t border-ink/[0.08] font-mono text-[10px] text-ink/45">{{ filteredQuestions.length }}/{{ totalQ }} questions</div>
          </aside>
          ...
          <main class="rev-zone relative flex-1 min-w-0 overflow-y-auto rounded-xl bg-white p-6 shadow-[0_14px_40px_-22px_rgba(35,32,58,0.3)] ring-1 ring-ink/[0.06]">
            <template v-if="q">
              <div class="sticky -top-6 -mx-6 -mt-6 px-6 py-3.5 bg-white/95 backdrop-blur border-b border-ink/[0.08] flex items-center justify-between gap-3 z-10 rounded-t-xl">
                <div class="font-mono text-[11px] tracking-wider text-ink/50 uppercase truncate">Q{{ q.number }} · {{ q.type }} · {{ q.subject || 'no subject' }}<span v-if="q.pageNo" class="ml-2 bg-hlyellow/60 text-ink px-1.5 py-0.5 rounded normal-case">p.{{ q.pageNo }}</span></div>
                <div class="flex gap-2 shrink-0">
                  <button :disabled="!hasPrev" class="px-3 py-1.5 rounded-lg border-2 border-ink/12 text-xs font-bold text-ink/70 hover:border-pen hover:text-pen transition-colors disabled:opacity-40 disabled:pointer-events-none" @click="prev">← Prev</button>
                  <button :disabled="!hasNext" class="px-3 py-1.5 rounded-lg border-2 border-ink/12 text-xs font-bold text-ink/70 hover:border-pen hover:text-pen transition-colors disabled:opacity-40 disabled:pointer-events-none" @click="next">Next →</button>
                </div>
              </div>

              <input v-model="paper!.meta.title" class="mt-4 w-full border border-ink/12 rounded-lg px-3.5 py-2.5 font-semibold text-[15px] bg-paper focus:outline-none focus:ring-2 focus:ring-pen/40" placeholder="Paper title" />

              <div v-if="showRendered" class="mt-3 w-full border border-ink/12 rounded-xl p-3.5 text-[15px] leading-relaxed bg-white min-h-[104px] cursor-text" title="Click to edit raw text / LaTeX" @click="showRendered = false">
                <MathText :text="q.text || ''" />
              </div>
              <textarea v-else v-model="q.text" rows="4" class="mt-3 w-full border border-ink/12 rounded-xl p-3 text-[15px] bg-paper focus:outline-none focus:ring-2 focus:ring-pen/40 font-mono text-[13px]" placeholder="Question text (markdown + $LaTeX$)" @input="markDirty()"></textarea>

              <div class="mt-3 grid sm:grid-cols-2 gap-2.5">
                <label class="block">
                  <span class="text-xs font-semibold text-ink/60">Type</span>
                ...
                <label class="block">
                  <span class="text-xs font-semibold text-ink/60">Subject</span>
                  <input v-model="q.subject" class="mt-1 w-full border border-ink/12 rounded-lg px-2.5 py-2 text-sm bg-paper focus:outline-none focus:ring-2 focus:ring-pen/40" placeholder="Physics / Reasoning / …" @input="markDirty()" />
                </label>
                <label class="block">
                  <span class="text-xs font-semibold text-ink/60">Section</span>
                  <input v-model="q.section" class="mt-1 w-full border border-ink/12 rounded-lg px-2.5 py-2 text-sm bg-paper focus:outline-none focus:ring-2 focus:ring-pen/40" placeholder="A / Part-1 / …" @input="markDirty()" />
                </label>
                <label class="block">
                  <span class="text-xs font-semibold text-ink/60">Topic</span>
                  <input v-model="q.topic" class="mt-1 w-full border border-ink/12 rounded-lg px-2.5 py-2 text-sm bg-paper focus:outline-none focus:ring-2 focus:ring-pen/40" placeholder="Kinematics / Profit-Loss / …" @input="markDirty()" />
                </label>
              </div>

              <div v-if="q.options" class="mt-4 grid gap-1.5">
                <div class="flex items-center justify-between">
                  <span class="text-xs font-bold text-ink/70">Options</span>
                  <label class="flex items-center gap-1.5 text-xs font-medium text-ink/60 cursor-pointer">
                    <input type="checkbox" :checked="q.hasDiagram" @change="toggleDiagram(selIdx)" class="accent-pen" /> hasDiagram
                  </label>
                </div>
                <div :class="optionsCompact(q) ? 'grid sm:grid-cols-2 gap-2' : 'grid gap-1.5'">
                  <div v-for="(opt, oi) in q.options" :key="oi" :class="['rounded-xl border transition-colors', isCorrect(q, oi) ? 'border-correct/60 bg-correct/[0.06]' : 'border-ink/10 bg-paper hover:border-ink/25']">
                    <div class="flex items-center gap-2 px-2.5 py-2">
                      ...
                      <span v-if="showRendered" class="flex-1 min-w-0 text-sm text-ink cursor-text truncate" :title="'click to edit — ' + opt" @click="showRendered = false"><MathText :text="opt" /></span>
                      <input v-else :value="opt" @input="q.options![oi] = ($event.target as HTMLInputElement).value; markDirty()" class="flex-1 min-w-0 bg-transparent text-sm text-ink focus:outline-none" placeholder="option…" />
              ...
              <div class="mt-4 flex flex-wrap gap-2.5 items-center text-sm">
                ...
                <input v-model="q.answer" class="border border-ink/12 rounded-lg px-2.5 py-1.5 text-sm w-24 bg-paper focus:outline-none focus:ring-2 focus:ring-pen/40" placeholder="1/42/…" @input="markDirty()" />
                ...
                <span class="ml-auto text-xs font-semibold text-ink/60">Marks <input v-model.number="q.marks" type="number" class="w-14 border border-ink/12 rounded-lg px-2 py-1.5 bg-paper focus:outline-none focus:ring-2 focus:ring-pen/40" @input="markDirty()" /> Neg <input v-model.number="q.negativeMarks" type="number" class="w-14 border border-ink/12 rounded-lg px-2 py-1.5 bg-paper focus:outline-none focus:ring-2 focus:ring-pen/40" @input="markDirty()" /></span>
              </div>
              ...
              <div class="mt-5 border-t border-dashed border-ink/15 pt-3.5">
                ...
                <button class="mt-2.5 px-4 py-2 rounded-lg border-2 border-ink/12 text-xs font-bold text-ink/70 hover:border-pen hover:text-pen transition-colors" @click="cropPlaceholder(selIdx)">{{ q.hasDiagram ? '+ crop / add another' : 'Crop diagram' }}</button>
            ...
            <div v-else class="h-full grid place-items-center">
              <p class="text-sm text-ink/45">Select a question from the list.</p>
            </div>
          </main>

          <!-- c) RIGHT VALIDATION PANEL — collapsible -->
          <button v-if="!showPanel" @click="showPanel = true" class="rev-zone hidden lg:flex flex-col items-center gap-2 px-2 py-4 self-stretch rounded-xl bg-white ring-1 ring-ink/[0.06] hover:ring-pen/40 transition-all" title="Show validation panel">
            <span class="font-mono text-[10px] font-bold tracking-widest text-ink/50" style="writing-mode: vertical-rl;">VALIDATION</span>
          ...
          <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 -mr-80" leave-active-class="transition-all duration-200 ease-in" leave-to-class="opacity-0 -mr-80">
            ...
            <div class="px-4 py-3 border-b border-ink/[0.08] bg-paper/60 flex items-start justify-between gap-2">
              <div>
                <div class="font-mono text-[11px] uppercase tracking-[0.25em] text-ink/50">Validation</div>
                <div class="mt-1.5 flex gap-2 text-[11px] font-bold">
                  <span :class="errCount ? 'bg-redmargin/[0.12] text-redmargin' : 'bg-correct/[0.12] text-green-700'" class="px-2 py-0.5 rounded-full">{{ errCount }} errors</span>
                  <span :class="warnCount ? 'bg-hlyellow/50 text-ink' : 'bg-ink/[0.05] text-ink/45'" class="px-2 py-0.5 rounded-full">{{ warnCount }} warnings</span>
                </div>
              </div>
              <button @click="showPanel = false" title="Collapse panel" class="shrink-0 w-7 h-7 rounded-lg hover:bg-ink/5 grid place-items-center text-ink/45">»</button>
            </div>
            <div class="flex-1 overflow-y-auto p-2.5 space-y-1.5">
              ...
              <div v-else class="mt-8 text-center">
                ...
                <p class="mt-2.5 text-sm font-display font-bold">All questions pass.</p>
                <p class="text-xs text-ink/45 mt-1">No missing answers, options or subjects.</p>
              </div>
            </div>
          </aside>
          </transition>
