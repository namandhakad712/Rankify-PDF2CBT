...
// pdfBuffer can arrive async (Dexie read) after mount â€” render when it lands
    ...
    <div class="bg-white rounded-2xl w-full max-w-[1100px] max-h-[94vh] flex flex-col overflow-hidden shadow-[0_30px_80px_-30px_rgba(35,32,58,0.5)] ring-1 ring-ink/10">
      ...
      <div class="shrink-0 border-b border-ink/10 px-5 py-3 flex flex-wrap items-center gap-x-4 gap-y-2">
        <div class="text-sm font-bold font-display tracking-tight mr-auto">
          Crop diagram
          <span v-if="pdfDocRef" class="font-mono text-xs font-normal text-ink/45 ml-2">source PDF Â· {{ totalPages }} pages</span>
        </div>

        <!-- Page navigation -->
        <div v-if="pdfDocRef" class="flex items-center gap-1.5">
          <button class="px-2.5 py-1.5 rounded-lg border border-ink/15 text-xs font-bold text-ink/70 hover:border-pen hover:text-pen transition-colors disabled:opacity-40 disabled:pointer-events-none" :disabled="pageNum<=1" @click="goto(pageNum-1)">â€¹</button>
          ...
          <button class="px-2.5 py-1.5 rounded-lg border border-ink/15 text-xs font-bold text-ink/70 hover:border-pen hover:text-pen transition-colors disabled:opacity-40 disabled:pointer-events-none" :disabled="pageNum>=totalPages" @click="goto(pageNum+1)">â€º</button>
        </div>

        <button @click="emit('close')" class="w-8 h-8 rounded-full hover:bg-ink/5 grid place-items-center text-ink/60 text-lg leading-none">âœ•</button>
      </div>
      ...
      <div class="flex-1 min-h-0 flex flex-col lg:flex-row gap-4 p-4 md:p-5">
        ...
        <div class="relative w-full lg:w-[62%] rounded-xl border border-ink/12 bg-paper overflow-hidden" style="height: 68vh">
          <div v-if="loadingPage || (!pageSrc && pdfDocRef)" class="absolute inset-0 z-10 grid place-items-center bg-white/70 text-sm text-ink/55 font-medium">Rendering page {{ pageNum }}â€¦</div>
          <div v-if="!pdfDocRef" class="absolute inset-0 grid place-items-center text-sm text-ink/50 px-6 text-center">
            {{ props.pdfBuffer ? "Loading PDFâ€¦" : "No PDF loaded â€” upload it in Extract first" }}
          </div>
          <Cropper
            v-else-if="pageSrc"
            ref="cropperRef"
        ...
        <aside class="w-full lg:w-64 shrink-0 flex flex-col gap-4">
          <div>
            ...
            <div class="mt-2 rounded-xl border border-dashed border-ink/15 bg-paper min-h-36 grid place-items-center p-2.5">
              ...
              <span v-else class="text-xs text-ink/40 text-center px-4">drag the box â€” live preview lands here</span>
            </div>
          </div>
