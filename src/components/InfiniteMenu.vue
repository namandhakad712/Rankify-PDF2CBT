...
class InfiniteGridMenu {
  ...
  private init() {
    ...
    if (!this.gl) {
      ...
      this.gl = this.canvas.getContext('webgl', { antialias: true, alpha: false }) as any;
    }
    
    if (!this.gl) {
      console.warn('⚠️ WebGL not supported, InfiniteMenu will not render');
...
<template>
  <div class="relative w-full h-full">
    ...
    <template v-if="activeItem">
      ...
      <div
        :class="[
          'select-none absolute top-4 left-1/2 -translate-x-1/2 text-center px-4 transition-all duration-500 ease-in-out xl:hidden',
          isMoving
      ...
      >
        ...
        <p class="text-sm sm:text-base text-white/80 max-w-md mx-auto">
